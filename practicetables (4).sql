...
create table marks (sid integer primary key,sname varchar(30),score integer);
insert into marks values (1,'Tim',45),(2,'Potter',35),(3,'Harry',50),(4,'Jon',44),(5,'Jim',55),(6,'Tom',89);
select * from marks;
...
-- 12/11/24
-- To display average salary along with all columns using WINDOW Function 
-- CTE (Common Table Expression) : CTE teels us that instead of selecting from a table, you select from another select command

with cte as (select emp_id, first_name,last_name,dep_id,salary,max(salary)OVER(PARTITION BY dep_id) as max_salary from myemp)select * from cte where cte.salary=max_salary;

-- rank() function :
select * from marks;
insert into marks values(7,'Lara',56),(8,'Jack',59),(9,'Jill',56);
select * from marks;
select *,rank() over(order by score desc) as Ranks from marks;


-- dense_rank() function:
select *,dense_rank() over(order by score desc) as Ranks from marks;

-- Find employee who gets 3rd highest salay in the company
select * from myemp;
select *,dense_rank()over(order by salary)as TOP3rd from myemp;
with cte as(select *,dense_rank()over(order by salary desc)as Ranks from myemp) select * from cte where cte.ranks=3;


-- Find 2nd employee who joined the company
with cte as(select *,dense_rank()over(partition by dep_id order by hire_date)as Joined from myemp) select * from cte where cte.Joined=2;

-- Find employee who joined 1st in each department in the company
select * from myemp;
with cte as(select *,dense_rank()over(partition by dep_id order by hire_date)as Ranks from myemp) select * from cte where cte.Ranks=1;

select * from sales;
select *,dense_rank()over(partition by fiscal_year order by sale desc) as Ranks from sales;
select * from marks;
select *,row_number() over (order by score) as Index_No from marks;